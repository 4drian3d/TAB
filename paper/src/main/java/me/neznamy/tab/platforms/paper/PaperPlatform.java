...
import me.neznamy.tab.platforms.paper.features.PaperTabExpansion;
import me.neznamy.tab.platforms.paper.features.PerWorldPlayerList;
 ...
 */
public class PaperPlatform extends BackendPlatform {

    @Getter private final PaperPacketBuilder packetBuilder = new PaperPacketBuilder();
    @Getter private final PaperPipelineInjector pipelineInjector = new PaperPipelineInjector();
    ...
    public void loadPlayers() {
        for (Player p : getOnlinePlayers()) {
            TAB.getInstance().addPlayer(new PaperTabPlayer(p, getProtocolVersion(p)));
        }
    }

    @Override
    public void registerPlaceholders() {
        new PaperPlaceholderRegistry().registerPlaceholders(TAB.getInstance().getPlaceholderManager());
    }

    @Override
    public NameTag getUnlimitedNametags() {
        return new PaperNameTagX(plugin);
    }

    @Override
    public TabExpansion getTabExpansion() {
        if (placeholderAPI) {
            PaperTabExpansion expansion = new PaperTabExpansion();
            expansion.register();
            return expansion;
        }
        return null;
